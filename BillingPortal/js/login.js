// Login page functionality - Updated for PHP backend
...
async function handleLogin(form, userType) {
    ...
    try {
        ...
        if (!response.ok) {
            console.warn('Main login failed, trying simple login...');
            // Fallback to simple login if main login fails
            return await trySimpleLogin(username, password, userType, submitBtn, originalText);
        }
        ...
        try {
            ...
            console.warn('Trying simple login as fallback...');
            return await trySimpleLogin(username, password, userType, submitBtn, originalText);
        }
...
// Fallback simple login for testing
async function trySimpleLogin(username, password, userType, submitBtn, originalText) {
    try {
        ...
        try {
            ...
            console.error('Simple login JSON parse error:', jsonError);
        ...
        if (result.success) {
            ...
            showMessage(result.message || 'Invalid credentials. Try: admin/admin123 or demo/demo123', 'error');
        }
    } catch (error) {
